...
if __name__ == '__main__':
    ...
    cursor_l.execute("SELECT OPENID FROM USER_LOCATION GROUP BY OPENID ORDER BY COUNT(OPENID) DESC")
    ...
    for each_openid in openid_list:
        counter += 1
        cursor_l.execute("SELECT ID, RECORD_TIME FROM USER_LOCATION WHERE OPENID = %s ORDER BY RECORD_TIME",
                         (each_openid,))
        ...
        if need_delete_id:
            ...
            cursor_l.execute("DELETE FROM USER_LOCATION WHERE ID in %s", (tuple(need_delete_id),))
            conn_l.commit()
